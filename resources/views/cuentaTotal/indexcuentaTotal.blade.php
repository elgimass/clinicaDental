...
<table class="table table-borderless table-hover">
    ...
    @foreach ( $cuentas as $cuentats )
            <tr>
                @if ($cuentats -> paciente_id == $pacientes -> id)
                <th>{{$cuentats->id}}</th>
            ...
            <th hidden>Q {{number_format($cuentats->totalCargos, 2)}}</th>


            <th hidden>Q {{number_format($cuentats->totalAbonos, 2)}}</th>
            <th hidden>Q {{number_format($cuentats->deuda, 2)}}</th>
